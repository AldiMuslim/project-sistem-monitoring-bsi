...
<head>
    ...
    <title>Lupa Password BSI Inventory - Premium Version</title>
...
<body>
    ...
    <div class="premium-card w-full max-w-[550px] rounded-[60px] p-12 md:p-16 z-10 text-center my-8">
        ...
        <div class="mb-6">
            ...
            <div class="text-xs text-[#F2A900] font-bold uppercase tracking-widest mt-2">Minta Reset Password</div>
        </div>

        <p class="text-xs text-gray-400 leading-relaxed mb-6 px-2 text-justify md:text-center">
            Lupa password login Anda? Silakan masukkan alamat email yang terdaftar di bawah ini. Sistem akan otomatis
            mengirimkan tautan verifikasi untuk memperbarui password Anda.
        </p>

        @if (session('status'))
            <div
                class="bg-teal-500/20 border border-teal-500 text-teal-200 text-[10px] py-3 px-4 rounded-xl mb-4 uppercase tracking-widest text-left">
                {{ session('status') }}
            </div>
        @endif
        ...
        <form method="POST" action="{{ route('password.email') }}">
            ...
            <div class="relative text-left mb-6">
                ...
                <input type="email" name="email" value="{{ old('email') }}" required autofocus
                    ...
                    class="input-premium w-full rounded-2xl pl-14 pr-6 py-4 text-sm" autocomplete="username">
            </div>
